...
def prepair_data(path,window_x,window_y):
    ...
    df['t'] = df.t.apply(pd.Timestamp)
    
    df['dow'] = df.t.apply(lambda x: x.dayofweek)
    ## just select working days
    df = df[(df.dow<=4)&(df.dow>=0)]
    df = df.drop(['dow'],axis=1)

    df = df.pivot_table(index='t', columns='stick')

    ## select tickers not nan in final day
    columns = df.c.columns[~df.c.iloc[-1].isna()]
    df = df.iloc[:, df.columns.get_level_values(1).isin(columns)]


    df.v = df.v.interpolate(method='linear',limit_area='inside',limit_direction='both', axis=0)
    df.c = df.c.interpolate(method='linear',limit_area='inside',limit_direction='both', axis=0)
